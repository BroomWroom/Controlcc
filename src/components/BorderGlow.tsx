...
function parseHSL(hslStr: string): HSL {
  ...
  if (!match) return { h: 24, s: 95, l: 60 }; // Default to CodeChef Orange HSL
...
const BorderGlow = ({
  ...
  className = '',
  edgeSensitivity = 30,
  glowColor = '24 95 60', // CodeChef Orange HSL
  backgroundColor = 'var(--card-bg)', // dynamic theme variables
  borderRadius = 12, // match dashboard card border radius
  glowRadius = 35,
  glowIntensity = 0.8,
  coneSpread = 25,
  animated = true, // default to animate on mount
  colors = ['#ef4444', '#f97316', '#3b82f6'], // Red, Orange, Blue matching dark/light accents
  fillOpacity = 0.4,
  style = {},
}: BorderGlowProps) => {
  ...
  const combinedStyle = {
    ...(backgroundColor && backgroundColor !== 'var(--card-bg)' ? { '--card-bg': backgroundColor } : {}),
